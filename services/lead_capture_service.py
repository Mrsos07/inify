...
class LeadCaptureService:
    ...
    def generate_lead_capture_prompt(self, agent_custom_prompt: str = '') -> str:
        ...
        base_prompt = """
## قواعد جمع بيانات العملاء:

### التعرف على اهتمام العميل:
- إذا أبدى العميل اهتماماً واضحاً بعقار معين أو طلب معاينة → يمكن اقتراح التواصل
- لا تطلب رقم الجوال إلا إذا أبدى العميل اهتماماً حقيقياً وصريحاً
- لا تكن ملحاً، اسأل مرة واحدة فقط إذا كان السياق يستدعي ذلك

### قواعد الرد المهمة:
- لا تنهي كل رد بسؤال - اترك مجال للعميل يرد بحرية
- لا تسأل أكثر من سؤال واحد في الرد الواحد
- قدّم المعلومات المطلوبة أولاً، ثم إذا كان السياق مناسباً يمكن إضافة سؤال واحد
- إذا رفض العميل إعطاء رقمه، احترم قراره واستمر في المساعدة

### عند طلب المعاينة:
- إذا طلب العميل معاينة صراحةً → اسأله عن الوقت المناسب مرة واحدة
- إذا حدد وقتاً → أكد له: "تم حجز موعد المعاينة يوم [التاريخ] الساعة [الوقت] ✅"
"""
